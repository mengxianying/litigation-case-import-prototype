...
function renderRecords() {
  ...
  const rows = paginated.items.length ? paginated.items.map((task) => {
    ...
    return `<tr><td><input type="checkbox" data-task-select="${task.id}" ${checked} aria-label="选择${task.orderNo}" /></td><td>${task.orderNo}</td><td>${task.name}</td><td>${task.phone}</td><td>${escapeHtml(batchById(task.batchId)?.name || "-")}</td><td>${resultTag(task.result)}</td><td>${evidenceTag(task.evidence)}</td><td>${task.retry}${task.manualRetryCount ? ` + 人工${task.manualRetryCount}` : ""}</td><td>${task.updatedAt}</td><td><button class="link" data-action="detail" data-id="${task.id}">查看详情</button>${task.evidence === "已留存" ? '　<button class="link" data-action="detail" data-id="' + task.id + '">查看凭证</button>' : ""}</td></tr>`;
  }).join("") : '<tr><td colspan="10" class="empty">暂无符合当前筛选条件的任务</td></tr>';
