...
| 导入处理详情 | 订单结果、待补材料、异常文件三个页签；批量补传和当前筛选明细导出 |
...
导入处理详情作为当前批次的统一工作台，固定三个页签：

| 页签 | 展示内容 | 可操作项 |
| --- | --- | --- |
| 订单结果 | 已生效、待材料激活、导入失败、自动覆盖和重复待处理订单 | 查看解析、失败原因、查看覆盖记录、批量补传、导出当前筛选订单明细 |
| 待补材料 | 缺失的必填或非必填材料 | 勾选范围、批量补传、导出当前筛选待补材料明细 |
| 异常文件 | 无法识别订单归属或材料类别的文件 | 预览/下载、批量补传、按需同步重新识别、导出当前筛选异常文件明细 |
...
4. 已分配或已流转订单不可覆盖，写入批次处理明细并展示当前流转状态。
...
1. 批次列表“下载明细”下载批次处理明细，反映当前批次多次导入后的最新处理结果。
2. 导入处理详情的导出按钮随页签变化：订单结果导出订单处理明细，待补材料导出待补材料明细，异常文件导出异常文件明细。
3. 有勾选记录时导出勾选记录；未勾选时导出当前页签、当前筛选结果。
...
- [ ] 三个详情页签均支持导出当前筛选结果，导出字段与当前页签口径一致。
